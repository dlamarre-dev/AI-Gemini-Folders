...
* ♿ **Keyboard Accessible:** Folder and prompt headers are focusable and toggle with Enter/Space, with visible focus rings and WCAG AA contrast in dark mode.
...
* 🔍 **Instant Search:** Real-time (debounced) search bar across all saved conversations, with a one-tap clear button.
* ☁️ **Cloud Sync:** `storage.sync` keeps everything synchronized across all devices on your browser profile. Folder/prompt open-state stays local to each device, so collapsing a folder on one machine doesn’t ripple to the others.
...
* 💎 **Custom Gem / AI Link:** Configure a custom URL (Gemini Gem or any AI) — right-click or long-press the 💎 button to set it, click to toggle it on/off for the “New Conversation” button. *(Gemini Folders only; AI Folders uses per-service shortcut buttons, with the same right-click / long-press gesture to configure the local LLM URL.)*
