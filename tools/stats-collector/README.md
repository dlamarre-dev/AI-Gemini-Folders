...
4. Watch the progress log. The collector visits:
   - Listing page (weekly users)
   - `analytics/installs` (installs, uninstalls, breakdowns by country/language/OS)
   - `analytics/users` (user breakdowns, active versions)
   - `analytics/impressions` (impression count)
   per item, then commits the merged result.
5. On success you'll see `✅ committed` (or `no change` if data hasn't changed).
      ...
      "history": [
        {
          ...
          "uninstalls":   297,
          "impressions":  null,
          "installs_by_country": { "États-Unis": 18, "Espagne": 16 },
          "installs_by_language": { "espagnol": 26, "anglais (États-Unis)": 13 },
...
Entries are **appended** — existing history is never deleted.
