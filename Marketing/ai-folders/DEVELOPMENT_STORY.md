...
The `generate_af_locales.py` script handles this. It reads the 43 Gemini Folders locale files, applies a set of per-language transformation rules, and outputs AI Folders locale files. For most strings, the transformation is simple substitution ("Gemini" → the appropriate local AI term). For morphologically complex languages — Finnish, Hungarian, Greek, Serbian — the substitution breaks grammatical case. Those languages required per-key overrides written by hand.
